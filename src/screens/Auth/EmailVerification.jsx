        ...
        try {
            ...
            if (e.response.status === 400) {
                ...
                if (verificationCode === "") {
                    ...
                    setError("Inavalid verification code!")
                }
            }
        }
    };

    const resendVeriifactionToken = async () => {
        try {
            const response = await axios.post(`${backendUrl}/users/resend-verification-token`, { email })
            console.log(response)
            if (response.status === 200) {
                setResendTokenSuccess("Verification token sent to your mail sucessfully!")
            }
        } catch (e) {
            setIsSubmitting(false)
            if (e.response) {
                if (e.response.status === 400 || e.response.status === 422) {
                    setResendTokenError("Can't send verification token at this time!");
                }
            } else {
                console.error("Error response is undefined:", e);
            }
        }
    };
    return (
        <>
            <div className='w-full flex flex-col justify-center items-center min-h-screen'>
                <div className='flex items-center flex-col mb-7 gap-2 w-[340px] md:w-[400px] px-1 text-sm'>
                    <h1 className='p-700 text-xl'>Email verification</h1>
                    <p className='p-500 text-gray-500 text-center'>Insert the security code sent to your email in order to verify your email.</p>
                </div>
                <form className="w-[340px] md:w-[400px]" onSubmit={handleSubmit}>
                    {resendTokenError && (<p className='p-600 text-red-600'>{resendTokenError}</p>)}
                    {resendTokenSuccess && (<p className='p-600 text-green-600'>{resendTokenSuccess}</p>)}
                    <div className="flex flex-wrap -mx-3 mb-6">
                        <div className="w-full px-3 flex flex-col gap-1">
                            <input className="text-3xl text-center p-400 appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-4 px-5 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" type="number" value={verificationCode} onChange={(e) => { setVerificationCode(e.target.value) }} />
                            {error && (<p className='p-600 text-red-600'>{error}</p>)}
                        </div>
